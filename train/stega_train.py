...
from utils.Stega_trainer import Trainer
from utils.yml import *
from val import validate_stega
...
def main(config):
    ...
    elif opt.data.dataset == "coco":
        tdl,vdl = get_cocodl(opt)
    else:
        tdl,vdl = get_imagenet(opt)
    # base model and optimizer
    encoder,decoder = get_stega(opt.model)
    discriminator = Stega_Discriminator()
    lpips_alex = lpips.LPIPS(net="alex", verbose=False)


    # optimizer
    if opt.train.optimizer == "adamw":
        optimizer_de = AdamW(params=decoder.parameters(), lr=opt.lrd)
        if opt.model.frozen_upsample:
            optimizer_en = AdamW(params=filter(lambda p: p.requires_grad, encoder.parameters()), lr=opt.lre)
        else:
            optimizer_en = AdamW(params=encoder.parameters(), lr=opt.lre)
    else:
        # AdamW Optimizer
        optimizer_de = Adam(params=decoder.parameters(), lr=opt.lrd)
        if opt.model.frozen_upsample:
            optimizer_en = Adam(params=filter(lambda p: p.requires_grad, encoder.parameters()), lr=opt.lre)
        else:
            optimizer_en = Adam(params=encoder.parameters(), lr=opt.lre)
    ...
    if opt.resume:
        encoder,decoder,discriminator,steps_since_im_loss_activated = resume_stega(encoder,decoder,discriminator,opt.resume_path)
        
    # wrap decoder
    decoder = DeformWrapper(decoder,accelerator.device,aug_method=opt.noise.choice,sigma=opt.noise.sigma)
    tdl,vdl,encoder,decoder,discriminator,lpips_alex,optimizer_en,optimizer_de,optimizer_dis = accelerator.prepare(tdl,vdl,
                                                                                                encoder,decoder,discriminator,lpips_alex,
                                                                                                optimizer_en,optimizer_de,optimizer_dis,)
    trainer = Trainer(opt,optimizer_en,optimizer_de,optimizer_dis,encoder,decoder,discriminator,accelerator,lpips_alex)
    # base train loop
    metrics_updater = BatchMetricsAccumulator("stega")
    for i_epoch in range(start_epoch,total_epochs):
        with tqdm(initial=0,total=len(tdl),position=0,desc=f"{name}-epoch:{i_epoch}",disable=not accelerator.is_main_process) as pbar:
            for images, _ in tdl:
                ...
                b_loss_list,b_metric_list,image_dict = trainer.train_on_batch(clean_images,messages,activated_step=steps_since_im_loss_activated)
                metrics_updater.update(b_loss_list,b_metric_list)
                if b_metric_list[0]> 0.9:
                    steps_since_im_loss_activated += 1
                pbar.update(1)

        loss_dict,metric_dict = metrics_updater.compute_epoch_metric(method='mean')
        metrics_updater.reset()
        print("steps_since_im_loss_activated:",steps_since_im_loss_activated)
        ...
        if accelerator.is_main_process:
            ...
            if (i_epoch+1) % opt.train.save_fre == 0:
                state_dict = {
                    "steps_since_im_loss_activated":steps_since_im_loss_activated,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset',choices=['celeb','coco','imagenet'])
    parser.add_argument('--num',type=int)
    parser.add_argument('--noise_choice',choices=['OO','GN','rotation','affine','scaling_uniform','stega_combined'])
    parser.add_argument('--sigma',type=float)
    parser.add_argument('--batch_size',type=int)
    parser.add_argument('--resume_path',type=str)
    parser.add_argument('--lre',type=float)
