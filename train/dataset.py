...
def get_celebdl(args):
    ...
    s = time()
    data_dir = change_path("/mnt/shared/deepfake/CelebA-HQ/train_all")
...
def get_cocodl(args):
    ...
    transform = transforms.Compose(transform_pipe)
    s = time()
    data_dir = "/mnt/shared/coco2017"
    train_dir = change_path(data_dir + "/train2017")
    val_dir = change_path(data_dir + "/val2017")
    ...
    print(f"Finished. Loading took {time() - s:.2f}s")
    train_size = int(len(tds))
    val_size = int(len(vds))
    print(f"Train_size:{train_size},Val_size:{val_size}")
...
def get_imagenet(args):
    ...
    transform = transforms.Compose(transform_pipe)
    s = time()
    data_dir = change_path("/data/shared/ImageNet-1k/train")
    print(f"Loading image folder {data_dir} ...")
